...
- **Client Application**: Connects to the server and uses Claude to answer weather queries
...
- Uses Claude to understand your queries
...
1. **Client** sends a user query to Claude
2. **Claude** decides which weather tools to call based on the query
3. **Client** forwards tool calls to the **MCP Server** via MCP protocol
4. **MCP Server** fetches weather data from OpenWeatherMap API
5. **Client** sends tool results back to Claude
...
- **Client Integration**: How to connect MCP servers with Claude
